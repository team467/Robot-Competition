...
//   private final boolean allocatedChannels;
...
//   private final double leftSensorDistanceToRobotCenter;
...
//   /**
//    * Create an set of Ultrasonic Sensor instances. This is designed to supchannel
//    * the Daventech SRF04 and Vex ultrasonic sensors.
//    * 
//    * @param leftPingChannel                  The digital output channel that sends
//    *                                         the pulse to initiate the sensor
//    *                                         sending the ping. Assumes left side
//    *                                         is used for pinging
//    * @param rightPingChannel                 the right output channel is required
//    *                                         so that the right side will listen.
//    * @param leftEchoChannel                  The digital input channel that
//    *                                         receives the echo. The length of time
//    *                                         that the echo is high represents the
//    *                                         round trip time of the ping, and the
//    *                                         distance.
//    * @param rightEchoChannel                 The digital input channel that
//    *                                         receives the echo. The length of time
//    *                                         that the echo is high represents the
//    *                                         round trip time of the ping, and the
//    *                                         distance.
//    * @param leftSensorDistanceToRobotCenter  the horizontal distance along the
//    *                                         robot side
//    * @param rightSensorDistanceToRobotCenter the horizontal distance along the
//    *                                         robot side
//    * @param units                            The units returned in either inches
//    *                                         or millimeters
//    */
//   public DualUltrasonic(final int leftPingChannel, final int rightPingChannel, final int leftEchoChannel,
//       final int rightEchoChannel, final double leftSensorDistanceToRobotCenter,
//       final double rightSensorDistanceToRobotCenter, final Units units) {
...
//   /**
//    * Gets the distance vertically from the robot's side to the detected object. If
//    * there is no valid value yet, i.e. at least one measurement hasn't completed,
//    * then return 0.
//    * 
//    * @return double Vertical Range in inches of the target returned from the
//    *         ultrasonic sensor.
//    */
//   public double range() {
//     return range;
...
//   /**
//    * Gets the distance horizontally from the center of the robot's side to the
//    * detected object.
//    * 
...
//    *
//    * <p>
//    * d1 (b) = distance from sensor A to target C d2 (a) = distance from sensor B
//    * to target C
//    * 
//    * <p>
//    * Heron's Formula Area of trianble ABC is sqrt(s(s-a)(s-b)(s-c)) where c = c1 +
//    * c2 s = (a+b+c) / 2
//    * 
//    * <p>
//    * height = (Area * 2) / c Distance from sensor 1 to height line: c1 = sqrt(b^2
//    * - h^2) Distance from sensor 1 to height line: c2 = sqrt(a^2 - h^2)
//    * 
//    * <p>
//    * c2 = c - c1 d1 = (speed-of-sound * time) / 2 d2 = (speed-of-sound * time) -
//    * d1
//    */
//   public void periodic() {
//     if (validRange()) {
//       final double leftDistance = leftCounter.getPeriod() * SPEED_OF_SOUND_IN_INCHES_PER_SECOND / 2.0;
//       final double rightDistance = rightCounter.getPeriod() * SPEED_OF_SOUND_IN_INCHES_PER_SECOND - leftDistance;
//       final double svar = (leftDistance + rightDistance + distanceBetweenSensors) / 2;
//       final double area = Math
//           .sqrt(svar * (svar - leftDistance) * (svar - rightDistance) * (svar - distanceBetweenSensors));
//       vertical = 2.0 * area / distanceBetweenSensors;
//       horizontal = Math.sqrt((leftDistance * leftDistance) - (vertical * vertical)) - leftSensorDistanceToRobotCenter;
...
//    *
//    * <p>
//    * <b>DANGER WILL ROBINSON, DANGER WILL ROBINSON:</b> This code runs as a task
//    * and assumes that none of the ultrasonic sensors will change while it's
//    * running. If one does, then this will certainly break. Make sure to disable
//    * automatic mode before changing anything with the sensors!!
//    */
...
//         // lock list to ensure deletion doesn't occur between empty check and retrieving
//         // sensor
...
//    * allocated. If the system was running in automatic mode (round robin) when the
//    * new sensor is added, it is stopped, the sensor is added, then automatic mode
//    * is restored.
//    */
...
//   /**
//    * Turn Automatic mode on/off. When in Automatic mode, all sensors will fire in
//    * round robin, waiting a set time between each sensor.
//    *
//    * @param enable Set to true if round robin scheduling should start for all the
//    *               ultrasonic sensors. This scheduling method assures that the
//    *               sensors are non-interfering because no two sensors fire at the
//    *               same time. If another scheduling algorithm is preferred, it can
//    *               be implemented by pinging the sensors manually and waiting for
//    *               the results to come back.
//    */
//   public void automaticMode(final boolean enable) {
...
//        */
//       for (final DualUltrasonic sensor : transmitSensors) {
...
//       /*
//        * Clear all the counters (data now invalid) since automatic mode is disabled.
//        * No synchronization is needed because the background task is stopped.
//        */
//       for (final DualUltrasonic sensor : transmitSensors) {
//         sensor.leftCounter.reset();
//         sensor.rightCounter.reset();
...
//    * sensor. This only works if automatic (round robin) mode is disabled. A single
//    * ping is sent out, and the counter should count the semi-period when it comes
//    * in. The counter is reset to make the current value invalid.
//    */
//   public void ping() {
...
//   /**
//    * Check if there is a valid range measurement. The ranges are accumulated in a
//    * counter that will increment on each edge of the echo (return) signal. If the
//    * count is not at least 2, then the range has not yet been measured, and is
//    * invalid.
//    *
...
//    */
//   public void distanceUnits(final Units units) {
//     switch (units) {
